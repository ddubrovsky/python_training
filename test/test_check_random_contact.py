...
def test_contact_on_home_page_wit_db(app, db):
    ...
    for i in range(len(contact_from_db)):
        ...
        #assert contact_from_home_page[i].all_email_from_home_page == filter_email(contact_from_db[i])
        #assert contact_from_home_page[i].all_phones_from_home_page == merge_phones_like_on_page(contact_from_db[i])
